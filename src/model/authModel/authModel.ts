<<<<<<< HEAD:src/model/authModel/authModel.ts
import fs from "fs"
...
export default class authModel {
  ...
  async check(username: string) {
    ...
    */
    const sql: string = fs
      .readFileSync(
        path.join(__dirname, "src", "model", "authModel", "check.sql")
      )
      .toString()
  ...
  async user(username: string) {
    ...
    try {
      ...
      const sql: string = fs
        .readFileSync(
          path.join(__dirname, "src", "model", "authModel", "user.sql")
        )
        .toString()
  ...
  async verify(username: string, password: string) {
    try {
      const sql: string = fs
        .readFileSync(
          path.join(__dirname, "src", "model", "authModel", "verify.sql")
        )
        .toString()
...
>>>>>>> bcc3b103a25ef8f5fa2d9e3957df0b66c19015a9:src/model/Models/authModel/authModel.ts
